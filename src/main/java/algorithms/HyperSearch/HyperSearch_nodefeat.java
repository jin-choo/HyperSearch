...
public class HyperSearch_nodefeat {
	int faultE;
	int faultV;
	int faultP;
	int epsilonE;
	...
	List<Set<Integer>> nodeFeatList = new ArrayList<>();
	Map<Integer, Double> nodePairSimFeat = new HashMap<>();
	...
	public HyperSearch_nodefeat() {

	}

	public List<Double> runAlgorithm(String data, int numOutcomes, int faultE, int faultV, int faultP, double nodeFeature, int split, String validTest, boolean bigDataset) throws IOException {
		...
		if (validTest.startsWith("valid")) {
			reader = new BufferedReader(new FileReader(String.format("../data/ground_train_data/%s_split_%d.txt", data, split)));
		} else if (validTest.equals("test")) {
			reader = new BufferedReader(new FileReader(String.format("../data/ground_data/%s_split_%d.txt", data, split)));
		} else if (validTest.equals("val")) {
			reader = new BufferedReader(new FileReader(String.format("../data/val_train/%s_split_%d.txt", data, split)));
		}
		...
		if (validTest.equals("val")) {
			...
			if (validTest.startsWith("valid")) {
				reader = new BufferedReader(new FileReader(String.format("../data/unique_ground_train_data/%s_split_%d.txt", data, split)));
			} else if (validTest.equals("test")) {
				reader = new BufferedReader(new FileReader(String.format("../data/unique_ground_data/%s_split_%d.txt", data, split)));
			}
			while (((line = reader.readLine()) != null)) {
				if (line.isEmpty() || line.charAt(0) == '#' || line.charAt(0) == '%' || line.charAt(0) == '@') {
					continue;
		...
		if (nodeFeature != 0.0) {
			reader = new BufferedReader(new FileReader(String.format("../data/node_feat/%s.txt", data)));
			Set<Integer> nodeFeat;
			while (((line = reader.readLine()) != null)) {
				if (line.isEmpty() || line.charAt(0) == '#' || line.charAt(0) == '%' || line.charAt(0) == '@') {
					continue;
				}
				lineSplited = line.split(" ");
				lineSplitedLength = lineSplited.length;
				nodeFeat = new HashSet<>();
				for (int i = 0; i < lineSplitedLength; i++) {
					if (lineSplited[i].equals("1.0")) {
						nodeFeat.add(i);
					}
				}
				nodeFeatList.add(nodeFeat);
			}
			reader.close();

			Set<Integer> node1FeatSet;
			int node1size;
			double node12sim;

			if (!bigDataset) {
				for (int node1 = 0; node1 < nv; node1++) {
					node1FeatSet = nodeFeatList.get(node1);
					node1size = node1FeatSet.size();
					for (int node2 = node1 + 1; node2 < nv; node2++) {
						node12sim = computeNodePairSimFeat(node1FeatSet, nodeFeatList.get(node2), node1size);
						nodePairSimFeat.put(node1 * nv + node2, node12sim);
					}
				}
			}
		}
		...
		if (validTest.startsWith("valid")) {
			reader = new BufferedReader(new FileReader(String.format("../data/ground_valid_data/%s_split_%d.txt", data, split)));
		} else if (validTest.equals("test")) {
			reader = new BufferedReader(new FileReader(String.format("../data/test_data/%s_split_%d.txt", data, split)));
		} else if (validTest.equals("val")) {
			reader = new BufferedReader(new FileReader(String.format("../data/val_test/%s_split_%d.txt", data, split)));
		}
		...
		if (validTest.equals("test")) {
			...
			BufferedWriter writer = new BufferedWriter(new FileWriter(String.format("./test_cand/%s_split_%d_%d.txt", data, split, numOutcomes)));
			...
			writer = new BufferedWriter(new FileWriter(String.format("./test_cand_sup/%s_split_%d_%d.txt", data, split, numOutcomes)));
			...
			BufferedWriter writer = new BufferedWriter(new FileWriter(String.format("./val_cand/%s_split_%d_%d.txt", data, split, numOutcomes)));
			for (Integer hyperedgeCandidate : hyperedgeCandidates) {
				if (hyperedgeCandidate > 0) {
					writer.write(hashedList.get(hyperedgeCandidate).toString() + "\n");
				} else if (hyperedgeCandidate < 0) {
			...
			writer = new BufferedWriter(new FileWriter(String.format("./val_cand_sup/%s_split_%d_%d.txt", data, split, numOutcomes)));
			for (Double sup : hyperedgeCandidatesSup) {
				writer.write(sup.toString() + "\n");
			}
			writer.close();
			...
			BufferedWriter writer = new BufferedWriter(new FileWriter(String.format("./valid_cand/%s_split_%d_%d.txt", data, split, numOutcomes)));
			for (Integer hyperedgeCandidate : hyperedgeCandidates) {
				if (hyperedgeCandidate > 0) {
					writer.write(hashedList.get(hyperedgeCandidate).toString() + "\n");
				} else if (hyperedgeCandidate < 0) {
			...
			writer = new BufferedWriter(new FileWriter(String.format("./valid_cand_sup/%s_split_%d_%d.txt", data, split, numOutcomes)));
			for (Double sup : hyperedgeCandidatesSup) {
				writer.write(sup.toString() + "\n");
			}
			writer.close();
	...
	public double computeNodePairSimFeat(Set<Integer> node1FeatSet, Set<Integer> node2FeatSet, int node1size) {
		int node2size = node2FeatSet.size();
		int node12size = 0;
		if (node1size < node2size) {
			for (Integer node1Feat : node1FeatSet) {
				if (node2FeatSet.contains(node1Feat)) {
					node12size++;
				}
			}
		} else {
			for (Integer node2Feat : node2FeatSet) {
				if (node1FeatSet.contains(node2Feat)) {
					node12size++;
				}
			}
		}
		return 1d / (node1size + node2size - node12size) * node12size;
	}

	public double computeNodePairSimFeat(Set<Integer> node1FeatSet, Set<Integer> node2FeatSet) {
		int node1size = node1FeatSet.size();
		int node2size = node2FeatSet.size();
		int node12size = 0;
		if (node1size < node2size) {
			for (Integer node1Feat : node1FeatSet) {
	...
	public double calculateSize2HOISup(Integer HOINumber, Set<Integer> edgeSet) {
		double HOISup = 0.0;
		double simFeat;
		...
		if (nodeFeature != 0.0) {
			if (!bigDataset) {
				simFeat = nodePairSimFeat.get(HOINumber);
			} else {
				simFeat = computeNodePairSimFeat(nodeFeatList.get(HOINumber / nv), nodeFeatList.get(HOINumber % nv));
			}
			return HOISup * Math.pow(simFeat, nodeFeature);
		}
		return HOISup;
	}
	...
	public double[] calculateHOISupBound(List<Integer> HOI, int prefixLength, Set<Integer> edgeSet1) {
		...
		if (nodeFeature != 0.0) {
			double sumNodePairSimFeat = 0.0;
			int countNodePairSimFeat = 0;
			if (!bigDataset) {
				for (List<Integer> nodePair : generateCombinations(HOI, 2)) {
					sumNodePairSimFeat += nodePairSimFeat.get(nodePair.get(0) * nv + nodePair.get(1));
					countNodePairSimFeat++;
				}
			} else {
				for (List<Integer> nodePair : generateCombinations(HOI, 2)) {
					sumNodePairSimFeat += computeNodePairSimFeat(nodeFeatList.get(nodePair.get(0)), nodeFeatList.get(nodePair.get(1)));
					countNodePairSimFeat++;
				}
			}
			simFeat = sumNodePairSimFeat / (double) countNodePairSimFeat;
		}
		if (prefixLength+1 >= faultE) {
			int support1 = edgeSet1.size();
			double bound = (HOISup + boundFunction1(HOI, prefixLength, edgeSet1, support1))* Math.pow(simFeat, nodeFeature) * Math.pow(simFeat, nodeFeature);
			if (bound >= HOIMinsup[prefixLength-2]) {
				HOISup = calculateSupport(HOI, prefixLength, edgeSet1, support1, HOISup) * Math.pow(simFeat, nodeFeature);
			}
			return new double[]{HOISup, bound};
		}
		return new double[]{HOISup * Math.pow(simFeat, nodeFeature), HOISup * Math.pow(simFeat, nodeFeature)};
	}
	...
	public double[] calculateHOI12SupBound(List<Integer> HOI, List<Integer> prefix, int prefixLength, int node1, Set<Integer> edgeSet1, int node2, Set<Integer> edgeSet2, Set<Integer> edgeSet12, double bound1, double bound2) {
		...
		if (prefixLength+2 >= faultE) {
			int support12 = edgeSet12.size();
			double bound = (HOISup + boundFunction12(prefix, prefixLength, node1, edgeSet1, node2, edgeSet2, edgeSet12)) * Math.pow(simFeat, nodeFeature);
			if ((prefixLength == 1 || (bound1 >= HOIMinsup[prefixLength-2] && bound2 >= HOIMinsup[prefixLength-2])) && bound >= HOIMinsup[prefixLength-1]) {
				HOISup = calculateSupport(HOI, prefixLength+1, edgeSet12, support12, HOISup) * Math.pow(simFeat, nodeFeature);
			}
			return new double[]{HOISup, bound};
		}
		return new double[]{HOISup * Math.pow(simFeat, nodeFeature), HOISup * Math.pow(simFeat, nodeFeature)};
	...
	public double[] calculateHOISoftSupSimFeat(List<Integer> HOI12, int prefixLength, Set<Integer> edgeSet12) {
		double HOISup = 0.0;
		double simFeat = 1.0;
		for (Integer edgeNum : edgeSet12) {
			HOISup += ((double) prefixLength+2.0) / (double) groundLen.get(edgeNum);
		...
		if (nodeFeature != 0.0) {
			...
			int countNodePairSimFeat = 0;
			if (!bigDataset) {
				for (List<Integer> nodePair : generateCombinations(HOI12, 2)) {
					sumNodePairSimFeat += nodePairSimFeat.get(nodePair.get(0) * nv + nodePair.get(1));
					countNodePairSimFeat++;
				}
			} else {
		...
		return new double[]{HOISup, simFeat};
	}
	...
	public void processEquivalenceClass(List<Integer> prefix, int prefixLength, List<Integer> equivalenceClassNodes, List<Set<Integer>> equivalenceClassEdgeSets) {
		...
		if (equivalenceClassNodesSize == 1) {
			...
			if (prefixLength == 1) {
				...
				if (!ground2.contains(HOINumber)) {
					HOISup = calculateSize2HOISup(HOINumber, equivalenceClassEdgeSets.get(0));
		...
		if (equivalenceClassNodesSize == 2) {
			...
			if (prefixLength == 1) {
				...
				if (!ground2.contains(HOINumber)) {
					HOISup = calculateSize2HOISup(HOINumber, edgeSet1);
					HOISupport0 = HOISupport.get(0);
					if (!HOISupport0.containsKey(HOISup)) {
						HOISupport0.put(HOISup, new HashSet<>());
					}
				...
				if (!ground2.contains(HOINumber)) {
					HOISup = calculateSize2HOISup(HOINumber, edgeSet2);
					HOISupport0 = HOISupport.get(0);
					if (!HOISupport0.containsKey(HOISup)) {
						HOISupport0.put(HOISup, new HashSet<>());
					}
			...
			if (prefixLength+2 <= maxLenHyperedge) {
				...
				if (bound1 >= HOIMinsupBound) {
					...
					for (int i2 = i1+1; i2 < equivalenceClassNodesSize; i2++) {
						...
						edgeSet12 = new HashSet<>(edgeSet1);
						edgeSet12.retainAll(edgeSet2);
						HOI12 = new ArrayList<>(prefix);
						HOI12.add(node1);
						HOI12.add(node2);
						Collections.sort(HOI12);
						HOISoftSupSimFeat = calculateHOISoftSupSimFeat(HOI12, prefixLength, edgeSet12);
						HOISup = HOISoftSupSimFeat[0];
						simFeat = HOISoftSupSimFeat[1];
						if (HOISup * Math.pow(simFeat, nodeFeature) >= HOIMinsup[prefixLength-1]) {
							...
							if ((HOISup + boundFunction12(prefix, prefixLength, node1, edgeSet1, node2, edgeSet2, edgeSet12)) * Math.pow(simFeat, nodeFeature) >= HOIMinsupBound12) {
								equivalenceClassISuffixNodes.add(node2);
								equivalence1EdgeSets.add(edgeSet12);
							}
						}
